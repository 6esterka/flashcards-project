...
export default function Stats(){
    ...
    const totalMastery= useMemo(()=>{
        ...
        const allLearnedCardsCount=allCards.filter((card)=>card.isLearned).length;
        const percent = total>0?Math.round(allLearnedCardsCount/total*100):0;
        return percent;
    },[decks]);
    return (    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
           <p className="text-slate-400 text-sm font-medium">{uiText.stats.totalMasteryTitle}</p>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 h-[400px]">
        <h2 className="text-xl font-bold mb-6 text-slate-800">{uiText.stats.learningProgressStatTitle}</h2>
        <ResponsiveContainer width="100%" height="90%">
          <BarChart data={chartData}>
            ...
            {/* Learned */}
            <Bar dataKey="learned" stackId="a" fill="#556cd6" radius={[0, 0, 4, 4]} />
            {/* Unlearned */}
            <Bar dataKey="unlearned" stackId="a" fill="#e76f51" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>)
